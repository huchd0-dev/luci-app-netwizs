...
return view.extend({
    ...
    render: function () {
        ...
        var htmlTemplate = [
            ...
            '  .nw-step-line svg { width: 16px; height: 16px; display: block; background: transparent !important; }',
        ...
        function updateStatusDisplay(isSilent) {
            try {
                ...
                Promise.all([ safePromise(uci.load('network'), null), safePromise(uci.load('dhcp'), null), safePromise(uci.load('wireless'), null), safePromise(getWanStatus(), {}) ]).then(function(results) {
                    ...
                    var devMgrBtn = "";
                    if (!isBypass) {
                         // AP 模式下不分配 IP，所以只有主/二级路由模式下才显示设备管理
                         var devUrl = window.location.pathname.replace('/netwiz', '/netwiz_dev');
                         devMgrBtn = "<div style='margin-top: 20px; width: 100%;'><a href='" + devUrl + "' style='font-size: 18.5px; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; background: rgba(255, 255, 255, 0.2); border: 1px solid rgba(255, 255, 255, 0.4); border-radius: 8px; color: #fff; text-decoration: none; font-weight: bold; transition: all 0.25s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.1);' onmouseover=\"this.style.background='rgba(255,255,255,0.3)'; this.style.transform='translateY(-2px)';\" onmouseout=\"this.style.background='rgba(255,255,255,0.2)'; this.style.transform='none';\"><svg style='width: 18px; height: 18px;' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><rect x='2' y='3' width='20' height='14' rx='2' ry='2'></rect><line x1='8' y1='21' x2='16' y2='21'></line><line x1='12' y1='17' x2='12' y2='21'></line></svg> 终端设备与 IP 绑定</a></div>";
                    }
