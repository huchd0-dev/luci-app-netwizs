...
return view.extend({
    ...
    render: function () {
        ...
        var htmlTemplate = [
            ...
            '.nw-wrapper { display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 101vh; padding-top: 4vh; padding-bottom: 10vh; font-family: -apple-system, BlinkMacSystemFont, sans-serif; }',
            '.nw-header { text-align: center; margin-bottom: 40px; background-color: #5e72e4; padding: 25px; margin-top: 3px; border-radius: 16px; position: relative; width: 100%; max-width: 750px; box-sizing: border-box; box-shadow: 0 10px 25px rgba(94, 114, 228, 0.15); z-index: 20; }',
            ...
            '  .nw-header { margin: 0 auto 0 !important; padding: 20px 15px !important; width: 100% !important; max-width: 320px !important; border-radius: 12px; }',
    ...
    bindEvents: function (container) {
        ...
        function updateStatusDisplay(isSilent) {
            try {
                ...
                Promise.all([ safePromise(uci.load('network'), null), safePromise(uci.load('dhcp'), null), safePromise(getWanStatus(), {}) ]).then(function(results) {
                    ...
                    // 物理网卡的真实 IP 并存入全局变量，用于外网/局域网冲突防呆检测
        ...
        container.querySelector('#btn-next-2').addEventListener('click', function () {
            try {
                ...
                // 第二道防线：读取底层 UCI 配置进行高级校验（冲突、重复提交等）
                uci.load('network').then(function() {
                    try {
                        ...
                        // 全局 WAN/LAN 网段冲突拦截：只有在真正获取到有效外网 IP 时才比对
        ...
        container.querySelector('#btn-apply').addEventListener('click', function () {
            ...
            var actionDetail = "";
            if (selectedMode === 'router' && rType === 'dhcp') {
                actionDetail = '<b style="color:#10b981;">' + T['OPT_DHCP'] + '</b>';
            } else {
