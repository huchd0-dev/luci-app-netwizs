...
**NetWiz**网络设置向导-极简，开箱即用，是一款逻辑严谨缜密，搭载企业级Lan口防失联机制，智能WAN口协议切换，智能防呆校验，智能IP、网关、WAN口、Lan口冲突排雷，前端交互体验行云流水、观感丝滑，彻底摆脱传统「点保存就异常、各类配置冲突频发」的糟糕使用体验。
...
### 📶 Powerful Smart Wi-Fi Engine (New!)
...
### 🛡️ 拒绝失联安全机制
* **安全回退机制：** 配置错误导致无法连接后台？不用怕！应用新配置后会启动 120 秒安全倒计时。LAN 口配置后若无真实的新 IP 访问连接，系统将自动恢复到修改前的配置，即使**意外断电重启依然有效**。
...
### 📶 强大的无线WiFi智控引擎 (New!)
* **无限芯片阵列探测：** 彻底打破传统插件单/双芯片的死板限制。自动扫描并接管系统内所有物理无线网卡，无论是单频、双频还是三频（甚至多频）路由器，均能完美适配，不闲置任何硬件。
* **底层洗白：** 针对各种残缺、冲突或被带偏的底层配置（如单芯片被锁死在 5G 模式），无视硬件历史残留，强行清洗并重写正确的物理协议与通道。
* **智能频宽满血释放：** 告别残血网络！在应用配置时，智能识别硬件极限，静默自动拉升频宽（如 2.4G 提至 HT40，5G 提至 VHT80），不破坏现有的 Wi-Fi 6 (HE) 配置，让测速轻松跑满。

### ⚡ 极致的无感体验
* **智能 WAN 口协议检测与切换：** 敏锐识别网线的物理插拔状态，完美过滤由拨号失败、软件重启引起的“假性掉线”。更换网线或切换接口时，系统自动感知并智能适配 **（拔除网线间隔十秒再接入，智能自动检测切换机制即会启用）**。
...
* 
### 💻 现代化前端架构
* **纯前端渲染 (CSR)：** 采用现代化的响应式、弹性架构构建（Flexbox 动态卡片排版），告别传统路由器后台卡顿的页面刷新，极速加载，操作响应极其迅速。
